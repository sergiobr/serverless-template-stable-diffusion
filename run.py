...
def main(opt):
    ...
    logging.log(logging.INFO, "opt " + str(opt))

    import json

    # Read settings files
    def load_file_args(path):
        print("load_file_args()", path)
        logging.log(logging.INFO, "load_file_args()" + path)

        with open(path, "r") as f:
            loaded_args = json.load(f)  # , ensure_ascii=False, indent=4)
            print("loaded_args", loaded_args)
            logging.log(logging.INFO, "load_file_args()" + str(loaded_args))
        return loaded_args

    print("opt", opt)
    logging.log(logging.INFO, "opt" + str(opt))

    logging.log(logging.INFO, "opt.settings" + opt.settings)
    master_args = load_file_args(opt.settings)
    logging.log(logging.INFO, "master_args" + str(master_args))
    ...
    if opt.enable_animation_mode:
        print("opt", opt)
        print("opt.enable_animation_mode", opt.enable_animation_mode)
        print("master_args", master_args)
        animation_prompts = master_args["animation_prompts"]
    else:
        animation_prompts = {}

    override_settings_with_file = True  # @param {type:"boolean"}
    settings_file = opt.settings# "custom" #@param ["custom", "512x512_aesthetic_0.json","512x512_aesthetic_1.json","512x512_colormatch_0.json","512x512_colormatch_1.json","512x512_colormatch_2.json","512x512_colormatch_3.json"]
    print("settings_file", settings_file)
    custom_settings_file = "./examples/runSettings_Animation.txt"  # @param {type:"string"}
    print("custom_settings_file", custom_settings_file)
    ...
    def DeforumArgs():
        ...
        sampler = master_args[
            "sampler"]  # @param ["klms","dpm2","dpm2_ancestral","heun","euler","euler_ancestral","plms", "ddim", "dpm_fast", "dpm_adaptive", "dpmpp_2s_a", "dpmpp_2m"]
    ...
    if override_settings_with_file:
        load_args(args_dict, anim_args_dict, settings_file, custom_settings_file, verbose=True)
    ...
    image_path = "./input/2022-09/20220903000939_%05d.png"  # @param {type:"string"}
    mp4_path = "./input/2022-09/20220903000939.mp4"  # @param {type:"string"}
    render_steps = False  # @param {type: 'boolean'}
    path_name_modifier = "x0_pred"  # @param ["x0_pred","x"]
    make_gif = False

    if skip_video_for_run_all == True or opt.enable_animation_mode == False:
        print('Skipping video creation', skip_video_for_run_all, opt.enable_animation_mode)
    else:
        import os
        import subprocess
